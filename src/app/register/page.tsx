...
function RegistrationForm() {
  ...
  const onSubmit = async (data: RegistrationFormValues) => {
    ...
    try {
      ...
      // Step 2: Update the user's display name in Auth. This is what the server-side function will use.
      await updateProfile(user, { displayName: data.name });

      // Step 3: THIS IS THE CRITICAL FIX.
      // The user document is now created by a secure server-side Cloud Function (`onUserCreate`).
      // The client's only job is to create the Auth user and handle the UI.
      // We will, however, create the user document here as well as a fallback, and to ensure
      // the referrer code from the form is captured. The server-side function will act as the
      ...
      // This `setDoc` will be secured by the new Firestore rule.
      // It allows a user to write to their own document upon creation.
      await setDoc(userDocRef, {
          ...
          pgcBalance: 0,
          referredBy: data.referredBy || 'ADMIN_ROOT_USER', // Capture from form
          // The referralCode will be generated by the onUserCreate Cloud Function
          // but we can set a placeholder or leave it to the function.
          walletPublicKey: null,
          isVerified: false,
          status: finalRole === 'User' ? 'Active' : (role ? 'Pending' : 'Active'),
          role: finalRole,
